...
import { meters } from './Finder.jsx'
import { UnitCard, eok, pct0, ratioTone } from './UnitLookup.jsx'
import { htName, useCompare, useFinder, useGuard, useSubway, useUnitLoader } from './units.js'
...
export default function Nearby({ guNames, region = '11', onBack }) {
  ...
  useEffect(() => { setLimit(PAGE); setOpen(null) }, [here, radius, ht])
  ...
  const pins = useMemo(() => {
    ...
    return hits.slice(0, 300).map(({ i }) => ({
      ...
      label: `${col.name[i] || col.jibun?.[i] || ''} · ${eok(col.jeonse[i])}`,
    }))
  }, [fin, hits])
  ...
  return (
    <section className="card">
      ...
      <p className="sub">
        지금 서 계신 곳 주변에서 최근 전세 계약이 있었던 건물을 가까운 순으로
        보여 드립니다. 위치는 이 기기에서만 쓰고 어디로도 보내지 않습니다
      </p>
      ...
      {!here ? (
        ...
        <>
          <div className="nb-ctl">
            ...
            <div className="filters">
              {HTS.map(([v, label]) => (
                <button key={v || 'all'} aria-pressed={ht === v} onClick={() => setHt(v)}>{label}</button>
              ))}
            </div>
          </div>
          ...
          {view === 'map' ? (
            ...
            <p className="muted-line">
              반경 안에 최근 전세 계약이 있었던 건물이 없습니다. 반경을 넓혀 보시거나,
              이 동네가 아직 수집 범위(서울·경기) 밖일 수 있습니다.
            </p>
          ) : (
      ...
      <p className="muted-line">
        매물 목록이 아닙니다. 최근 2년 전세 신고가 있었던 건물이며, 지금 계약할 수
        있는 방인지는 알 수 없습니다. 눈앞 건물을 눌러 판정을 확인하고 비교함에
        담아 두시면 집에 돌아가 나란히 견주실 수 있습니다.
      </p>
    </section>
  )
}
