 ...
 *
 * 파이프라인이 둘이고 주기가 다르다. 실거래는 주간 크론(월 21:00 UTC = 화
 * 06:00 KST)이고 건축물대장은 매일 밤 크론(13:07 UTC = 22:07 KST)이다. 그래서
 * 재는 함수도 collectLate와 bldgLate로 따로 둔다. 화면 문구는 반드시 "실거래"
 * 또는 "건축물대장"이라고 주어를 붙여 말한다. 둘을 "갱신" 한 단어로 뭉치면
 * 건물 정보까지 실거래 날짜 것으로 읽힌다.
 */
...
/* KST 달력일 번호. 절대 24시간이 아니라 이것으로 세는 이유는 collectLate 주석에
   적어 두었다. 화면이 날짜를 함께 찍으므로 사용자가 직접 빼는 값과 맞아야 한다. */
...
/**
 * 건축물대장이 며칠째 멈춰 있으면 {days, got}, 아니면 null.
 *
 * 실거래와 달리 주기가 매일이라 놓친 예정 시각(due)을 짚어 말하지 않는다.
 * 매일 도는 것에 "8월 28일에 받았어야 할"이라고 적으면 그날 하루만의 문제로
 * 읽힌다. 마지막으로 받은 날만 말한다.
 *
 * 두 인자가 다 있어야 판정한다.
 *
 *   at         수집기가 마지막으로 기록을 남긴 시각(bldg_log.fetched_at 최댓값)
 *   remaining  마지막 회차가 끝난 뒤의 잔여 대상 지번 수(bldg_meta)
 *
 * remaining이 0이면 멈춘 것이 아니라 다 받은 것이다. 그날부터 수집기는 계획이
 * 비어 즉시 종료하므로 시각이 영영 안 움직인다. 이 가드가 없으면 완주하는
 * 날부터 없는 장애를 매일 알린다. remaining이 null이면(옛 산출물이라 표식이
 * 없다) 판정을 접는다. 근거가 없을 때는 조용한 쪽으로 무너뜨린다.
 *
 * 임계는 KST 달력일 차 2다. 1이 아닌 이유: 크론이 22:07 KST에 시작해 최장
 * 5시간 15분을 도므로 한 회차가 KST 자정을 넘겨 다음 날짜로 기록된다. 게다가
 * 8/27 회차는 9시간 39분 늦게 들어오고도 같은 KST 달력일 안에 끝났다. 1로
 * 잡으면 정상 회차마다 낮 동안 거짓 경고가 뜬다. 2면 최소 한 밤을 확실히
 * 건너뛴 뒤에만 말한다. 늦게 알아차리는 비용은 하루치 지연뿐이다.
 *
 * 남은 창이 하나 있다. 회차가 KST 자정 전에 끝나면(연속 실패 중단이나 쿼터
 * 조기 소진) at이 그날 22시대가 되고, 다음 회차가 도는 00:00~04:00 사이에 한
 * 밤도 안 건너뛴 채로 "2일째"가 뜬다. 달력일 경계를 자정이 아니라 새벽 5시로
 * 옮기면 닫히지만, 그러면 days가 화면에 찍힌 두 날짜의 차와 어긋나 사용자가
 * 직접 빼는 값과 안 맞고, 8/27식 지각 회차에서 도리어 새 거짓 경고가 생긴다.
 * 창이 새벽 네 시간이고 방향이 안전한 쪽이라 그대로 둔다.
 */
/**
 * 화면에 적어도 되는 대장 수집 시각. 미래면 null이다.
 *
 * 이 값의 기준 시계는 서버가 아니라 이용자 기기다. 정적 산출물이라 그렇다.
 * 기기 시계가 뒤로 밀려 있으면 "마지막으로 받은 것은 9월 1일입니다"를 8월에
 * 단언하게 되고, 그때 bldgLate는 days가 음수라 조용해지므로 지연 경고까지
 * 함께 꺼진다. 방향이 안전한 쪽이 아니다. 미래면 아예 말하지 않는다.
 *
 * 하루는 봐 준다. 회차가 방금 끝난 순간과 기기 시계의 작은 어긋남까지 잡으면
 * 멀쩡한 날짜가 통째로 사라진다.
 */
export function bldgAt(at, now = new Date()) {
