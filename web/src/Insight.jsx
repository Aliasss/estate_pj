  ...
  return (
    <>
      <section className="card">
        ...
        {/* 왜 최신 달이 통계에 없는지를 화면이 말한 적이 없었다. 8월 말에 앱을
            열면 확정 구간이 6월이라 낡아 보이는데, 그건 우리가 게을러서가
            아니라 신고 기한이 30일이라 그때까지 자료가 채워지는 중이어서다.
            이유를 적으면 낡음이 아니라 구조로 읽힌다.

            층을 둘로 갈라 적는다. 초안은 "최근 두 달은 월 통계에 넣지 않는다"고
            썼는데 거짓이었다. 지난달은 통계에 들어가고 위 문장이 바로 잠정이라고
            말하며 차트에도 회색으로 그려진다. 안 넣는 것은 진행 중인 이번 달
            하나뿐이다. "최근 두 달"이라는 셈도 틀린다. 매달 1일부터 5일까지는
            기한이 안 지난 달이 셋이다. 숫자를 안 쓴다. */}
        <p className="ins-when">
          부동산 거래는 계약일로부터 30일 안에 신고하게 되어 있습니다. 그래서
          신고 기한이 지나지 않은 달은 <b>잠정</b>으로 표시하고 증감률을 내지
          않습니다. 진행 중인 이번 달은 아직 계약이 일어나는 중이라 월 통계에
          넣지 않고, 신고된 개별 거래만 <b>수집 회차마다</b> 물건 화면의 거래
          목록에 잠정 표시와 함께 실립니다.
        </p>
